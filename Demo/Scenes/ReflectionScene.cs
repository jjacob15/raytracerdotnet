...
namespace Demo.Scenes
{
    public class ReflectionScene : AbstractScene
    {
        public override void Initialize()
        {
            ...
            floor.Transform = Matrix.Transformation().Translation(0, 0.25, 0).Apply();
            ...
            right.Material = new Material(new GradientPattern(new Color(0, 0, 1), new Color(1, 0.5, 0.1)) { Transform = Matrix.Transformation().Scaling(0.25, 1, 1).RotateY(30).Apply() }, diffuse: 0.7, specular: 1);

            var left = new Sphere();
            left.Transform = Matrix.Transformation().Scaling(0.33, 0.33, 0.33).Translation(-1.5, 0.33, -0.75).Apply();
            left.Material = new Material(new CheckerPattern(new Color(1, 0, 0), new Color(0, 1, 0)) { Transform = Matrix.Transformation().Scaling(0.25, 0.25, 0.25).Apply() }, diffuse: 0.7, specular: 0.3);
